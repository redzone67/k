...
def build_baseline(use_file: str, import_file: str) -> BEABaseline:
    use = read_use_table(use_file)
    M_import = read_import_matrix(import_file, use["codes"])

    U_total = use["U_total"]

    # Keep negative domestic entries only as diagnostics for future import extension
    ...
    if neg_count > 0:
        ...
        for r, c in neg_idx:
            neg_entries.append({
                "row_code": use["codes"][r],
                "row_name": use["names"][r],
                "col_code": use["codes"][c],
                "col_name": use["col_names"][c],
    ...
    # Daily flows
    U_total_daily = U_total / DAYS_PER_YEAR
    ...
    # Technical coefficients from total use only
    A_total = safe_divide(U_total_daily, x0_daily.reshape(1, -1))

    # Inventories from total use only
    n_days = np.full(len(use["codes"]), INVENTORY_DAYS, dtype=float)
    S0_total = U_total_daily * n_days.reshape(1, -1)
    ...
    # Future extension only
    import_share_matrix = safe_divide(M_import, U_total)
    import_share_col = safe_divide(M_import.sum(axis=0), U_total.sum(axis=0))

    baseline = BEABaseline(
        codes=use["codes"],
        names=use["names"],
        U_total_annual=U_total,
...
def diagnostic_report(baseline: BEABaseline, use_file: str, import_file: str) -> Dict:
    """
    Main consistency checks for Step A-D.
    """
    ...
    use = read_use_table(use_file)
    industry_output_gap = row_balance - use["x0_industry"]
    ...
    diag = {
        ...
        "industry_output_gap_max_abs": float(np.abs(industry_output_gap).max()),
...
#def main() -> None:
print("Building baseline from BEA files...")
baseline = build_baseline(USE_FILE, IMPORT_FILE)

diag = diagnostic_report(baseline, USE_FILE, IMPORT_FILE)
